...
You can use "Target White" to calibrate whitepoint to a desirable value (D50, D65, D93, or Custom x, y). It is especially useful if you can not achieve a desirable whitepoint with the display's RGB gain controls.
...
The tool generates and applies an ICC profile (optional when a GPU driver API is used) that contains idealized display characteristics under applied color transformations. In other words, this profile describes the display as ideally matching the selected color space and gamma, allowing applications that support ICC profiles to deliver accurate and consistent results.

Supported GPU driver APIs: NVAPI (NVIDIA) and ADL (AMD).

# System requirements

The tool primarily relies on MHC2, and therefore its requirements align with those of the [Windows hardware display color calibration pipeline](https://learn.microsoft.com/en-us/windows/win32/wcs/display-calibration-mhc#system-requirements).
...
When MHC2 or ADL is used, the sRGB gamma option provides the best ΔE. Using different gamma settings, especially Absolute or Relative with a 100% black output offset, results in oversaturation/desaturation of colors depending on the display's native color space and its final gamma difference from sRGB. This comes from the lack of control over degamma, which is usually fixed to sRGB. It can be mitigated by enabling "Optimize CSC matrix for gamma".
...
Applying any kind of calibration usually results in banding unless dithering or at least 10-bit output is used for the display. However, even 10-bit displays can benefit from dithering, because modern Windows versions no longer limit the desktop to 8-bit.

For the best results, dithering output bit depth must match the display's connection bit depth. Using a lower bit depth will most likely produce worse results, while using a higher bit depth may be equivalent to disabling dithering.

In general, dithering simulates higher perceived bit depth by adding controlled noise. The exact effectiveness depends on the dithering method used. Though temporal methods are usually the most effective, they work by rapidly switching between colors, which some people's eyes are sensitive to.

Supported GPU driver APIs provide dithering control options. Changes to these options take effect immediately and do not depend on the tool's other settings.
...
* When NVAPI is used, the -104 error may occur in SDR, or clamp may take effect only under certain conditions. To fix this, try installing the latest driver with DDU or disable MPO.
